...
class AudioChannelSplitter:
    ...
    def __init__(
        ...
        if self.enable_audio_saving:
            try:
                ...
                logger.info(f"🎵 AudioChannelSplitter: Audio saving ENABLED")
                logger.info(f"   📁 Save path: {audio_save_path}")
                logger.info(f"   ⏱️  Duration: {save_duration}s")
            except Exception as e:
                logger.error(f"❌ AudioChannelSplitter: Failed to initialize audio saver: {e}")
                self.audio_saver = None
                self.enable_audio_saving = False
        
        logger.info(f"🔀 AudioChannelSplitter initialized: format={audio_format}, "
                   f"bytes_per_sample={self.bytes_per_sample}, silence_threshold={silence_threshold}")
        if self.enable_audio_saving:
            logger.info(f"🎵 AudioChannelSplitter: Audio saving enabled for debugging")
    ...
    def split_stereo_chunk(self, audio_chunk: bytes) -> SplitResult:
        ...
        try:
            ...
            if chunk_size % bytes_per_stereo_sample != 0:
                ...
                logger.error(f"   📊 Expected stereo sample size: {bytes_per_stereo_sample} bytes")
                logger.error(f"   📊 Remainder: {chunk_size % bytes_per_stereo_sample} bytes")
                return SplitResult(
                    left_channel=b'',
                    right_channel=b'',
                    left_metrics=ChannelMetrics(),
            ...
            if self.total_chunks_processed <= 5:
                ...
                logger.info(f"✅ CHANNEL SPLIT VALIDATION (chunk #{self.total_chunks_processed}):")
            ...
            if self.enable_audio_saving and self.audio_saver:
                if self.total_chunks_processed == 1:  # Start recording on first chunk
                    if self.audio_saver.start_recording():
                        logger.info(f"🎵 AudioChannelSplitter: Started saving split audio to files")
                ...
                if self.audio_saver.is_active:
                    ...
                    # Log write failures
                    if not left_written:
                        logger.warning(f"⚠️ AudioChannelSplitter: Failed to write left channel audio (chunk #{self.total_chunks_processed})")
                    if not right_written:
                        logger.warning(f"⚠️ AudioChannelSplitter: Failed to write right channel audio (chunk #{self.total_chunks_processed})")
            ...
            if self.total_chunks_processed <= 10:
                ...
                logger.info(f"   📊 Original: {chunk_size} bytes → Left: {len(left_channel)} bytes, Right: {len(right_channel)} bytes")
                logger.info(f"   🎚️  Left: {left_metrics.activity_level} (max: {left_metrics.max_amplitude}, avg: {left_metrics.avg_amplitude:.1f})")
                logger.info(f"   🎚️  Right: {right_metrics.activity_level} (max: {right_metrics.max_amplitude}, avg: {right_metrics.avg_amplitude:.1f})")
                if self.enable_audio_saving:
                    logger.info(f"   🎵 Audio saving: {'ACTIVE' if self.audio_saver and self.audio_saver.is_active else 'INACTIVE'}")
        ...
        try:
            async for audio_chunk in audio_stream:
                ...
                if split_result.split_successful:
                    ...
                    logger.error(f"❌ Failed to split chunk #{chunk_count}: {split_result.error_message}")
                    
        except asyncio.CancelledError:
            logger.info(f"🛑 Channel splitter cancelled after processing {chunk_count} chunks")
            raise
        except Exception as e:
            ...
            if self.enable_audio_saving and self.audio_saver and self.audio_saver.is_active:
                try:
                    ...
                    logger.info(f"🎵 AudioChannelSplitter: Audio saving completed")
                    if 'left_channel' in save_stats:
                        logger.info(f"   📁 Left file: {save_stats['left_channel'].get('file_path', 'N/A')}")
                        logger.info(f"   📁 Right file: {save_stats['right_channel'].get('file_path', 'N/A')}")
                except Exception as e:
                    logger.error(f"❌ AudioChannelSplitter: Error stopping audio saver: {e}")
    ...
    def _log_split_statistics(self, chunk_count: int) -> None:
        ...
        logger.info(f"   🔇 Left channel silence: {left_silence_rate:.1f}% ({self.left_silent_chunks}/{chunk_count})")
        logger.info(f"   🔇 Right channel silence: {right_silence_rate:.1f}% ({self.right_silent_chunks}/{chunk_count})")
        
        # Warn about potential issues
        if left_silence_rate > 80:
            logger.warning(f"⚠️ Left channel mostly silent ({left_silence_rate:.1f}%) - check Source A connection")
        if right_silence_rate > 80:
            logger.warning(f"⚠️ Right channel mostly silent ({right_silence_rate:.1f}%) - check Source B connection")
    ...
    def _log_final_statistics(self) -> None:
        ...
        logger.info(f"🔀 Final Channel Split Statistics:")
        ...
        if left_silence_rate > 50 and right_silence_rate > 50:
            logger.warning("⚠️ Both channels have high silence rates - check audio sources")
        elif left_silence_rate > 80:
            logger.warning("⚠️ Left channel (Source A) mostly silent - consider using only right channel")
        elif right_silence_rate > 80:
            logger.warning("⚠️ Right channel (Source B) mostly silent - consider using only left channel")
        else:
            logger.info("✅ Both channels have reasonable activity levels for dual transcription")
    ...
    def stop_audio_saving(self) -> Optional[Dict[str, Any]]:
        ...
        if self.enable_audio_saving and self.audio_saver and self.audio_saver.is_active:
            try:
                ...
                logger.error(f"❌ AudioChannelSplitter: Error stopping audio saving: {e}")
                return None
        return None
    ...
    def _log_split_results(self, split_result: 'SplitResult', left_metrics: 'ChannelMetrics', right_metrics: 'ChannelMetrics', original_size: int) -> None:
        ...
        if self.total_chunks_processed % 25 == 0:
            logger.info(f"🔀 CHANNEL SPLIT ANALYSIS (chunk #{self.total_chunks_processed}):")
            logger.info(f"   📊 Input: {original_size} bytes")
            logger.info(f"   📊 Output: Left={len(split_result.left_channel)} bytes, Right={len(split_result.right_channel)} bytes")
            ...
            if left_metrics.is_silent and right_metrics.is_silent:
                logger.warning(f"   ⚠️  BOTH CHANNELS SILENT - This explains why WAV files have no audio!")
            elif left_metrics.is_silent:
                logger.warning(f"   ⚠️  LEFT CHANNEL SILENT - Only right channel has audio")
            elif right_metrics.is_silent:
                logger.warning(f"   ⚠️  RIGHT CHANNEL SILENT - Only left channel has audio")