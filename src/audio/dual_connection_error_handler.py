...
class DualConnectionErrorHandler:
    ...
    def __init__(
        ...
        logger.info("🛡️ DualConnectionErrorHandler initialized:")
    ...
    def record_connection_attempt(self, channel: str) -> None:
        ...
        logger.debug(
            f"📊 {channel.title()} channel: Connection attempt #{metrics.connection_attempts}"
        )
    ...
    def record_connection_failure(self, channel: str, error: Exception) -> None:
        ...
        logger.error(
            f"❌ {channel.title()} channel: Connection failed (#{metrics.consecutive_failures}): {error}"
        )
    ...
    def record_result_received(
        ...
        logger.debug(
            f"📝 {channel.title()} channel: Result received (#{metrics.results_received})"
        )
    ...
    def _activate_fallback(
        ...
        logger.warning(
            f"🔄 Activated fallback mode: {strategy.value} using {active_channel} channel"
        )
    ...
    def _check_stale_connections(self, current_time: float) -> None:
        ...
        if (
            ...
            logger.warning(
                f"⚠️ Left channel: No recent activity ({current_time - self.left_metrics.last_success_time:.0f}s)"
            )
        ...
        if (
            ...
            logger.warning(
                f"⚠️ Right channel: No recent activity ({current_time - self.right_metrics.last_success_time:.0f}s)"
            )
    ...
    def _log_health_summary(self) -> None:
        ...
        logger.info(
            f"   🎚️  Right Channel: {self.status.right_health.value} "
            f"({self.right_metrics.results_received} results, {self.right_metrics.consecutive_failures} failures)"
        )
        logger.info(
            f"   🔄 Fallback: {self.status.fallback_active} "
            f"({'active on ' + self.status.active_channel if self.status.fallback_active else 'disabled'})"
        )

    def _log_final_statistics(self) -> None:
        """Log final statistics."""
        logger.info("📊 Final Dual Connection Statistics:")
        logger.info(f"   ⏱️  Total Uptime: {self.status.uptime:.1f}s")
        logger.info(
            f"   📊 Left Channel: {self.left_metrics.connection_attempts} attempts, "
            f"{self.left_metrics.successful_connections} success, {self.left_metrics.failed_connections} failed"
        )
        logger.info(
            f"   📊 Right Channel: {self.right_metrics.connection_attempts} attempts, "
            f"{self.right_metrics.successful_connections} success, {self.right_metrics.failed_connections} failed"
        )
        logger.info(
            f"   📝 Total Results: Left={self.left_metrics.results_received}, Right={self.right_metrics.results_received}"
        )
        logger.info(
            f"   📡 Data Sent: Left={self.left_metrics.bytes_sent:,} bytes, Right={self.right_metrics.bytes_sent:,} bytes"
        )
        logger.info(
            f"   🔄 Fallback Activations: {1 if self.status.fallback_active else 0}"
        )
