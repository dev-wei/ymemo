...
class AudioFileWriter:
    ...
    def start_recording(self) -> bool:
        ...
        with self._lock:
            if self.is_recording:
                logger.warning(f"⚠️ AudioFileWriter: Already recording to {self.file_path}")
                return False
        ...
        with self._lock:
            try:
                ...
                if self.bytes_written + len(audio_data) > self.max_bytes:
                    ...
                    logger.info(f"🎵 AudioFileWriter: Maximum duration reached, stopping recording")
                ...
                if self.bytes_written % (self.sample_rate * self.sample_width * 5) == 0:  # Every ~5 seconds
                    ...
                    logger.debug(f"🎵 AudioFileWriter: {elapsed_time:.1f}s recorded, {self.bytes_written:,} bytes written")
    ...
    def _stop_recording_internal(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🎵 AudioFileWriter: Recording stopped")
            ...
            logger.info(f"   📊 Data: {self.bytes_written:,} bytes, {self.total_samples:,} samples")
            ...
            if self.file_path.exists():
                try:
                    with wave.open(str(self.file_path), 'rb') as test_wave:
                        ...
                        logger.info(f"   ✅ File validation: {test_frames} frames, {test_rate}Hz, {test_channels}ch")
                except Exception as e:
                    logger.warning(f"   ⚠️ File validation failed: {e}")
            else:
                logger.error(f"   ❌ File was not created successfully")
...
class DualChannelAudioSaver:
    ...
    def __init__(
        ...
        logger.info(f"🎵 DualChannelAudioSaver: Initialized")
    ...
    def start_recording(self) -> bool:
        ...
        if left_started and right_started:
            self.is_active = True
            logger.info(f"🎵 DualChannelAudioSaver: Both channels recording started")
            return True
        else:
            logger.error(f"❌ DualChannelAudioSaver: Failed to start recording")
    ...
    def stop_recording(self) -> Dict[str, Any]:
        ...
        logger.info(f"🎵 DualChannelAudioSaver: Recording stopped for both channels")
