...
class DualChannelResultMerger:
    ...
    def __init__(
        ...
        logger.info("🔄 DualChannelResultMerger initialized:")
    ...
    async def add_left_result(self, result: TranscriptionResult) -> None:
        ...
        if result.confidence < self.confidence_threshold:
            logger.debug(
                f"🚫 Left channel: Dropped low confidence result ({result.confidence:.2f})"
            )
        ...
        logger.debug(
            f"📝 Left channel: Added result '{result.text}' (confidence: {result.confidence:.2f})"
        )
    ...
    async def add_right_result(self, result: TranscriptionResult) -> None:
        ...
        if result.confidence < self.confidence_threshold:
            logger.debug(
                f"🚫 Right channel: Dropped low confidence result ({result.confidence:.2f})"
            )
            self.stats.dropped_results += 1
            return
        ...
        logger.debug(
            f"📝 Right channel: Added result '{result.text}' (confidence: {result.confidence:.2f})"
        )
    ...
    async def _output_result(self, result: TranscriptionResult) -> None:
        ...
        logger.debug(
            f"🔄 Merged result: {result.speaker_id}: '{result.text}' "
    ...
    def _log_merge_statistics(self) -> None:
        ...
        logger.info(
            f"   📊 Input: Left={self.stats.total_left_results}, Right={self.stats.total_right_results}"
        )
        logger.info(
            f"   📤 Output: Merged={self.stats.merged_results}, Dropped={self.stats.dropped_results}"
        )
        logger.info(f"   ⚔️  Conflicts: {self.stats.conflicting_results}")
        logger.info(
            f"   📋 Buffer Status: Left={len(self.left_buffer)}, Right={len(self.right_buffer)}"
        )
    ...
    async def _flush_remaining_results(self) -> None:
        ...
        if remaining_results:
            logger.info(
                f"🔄 Result Merger: Flushed {len(remaining_results)} remaining results"
            )
    ...
    def _log_final_statistics(self) -> None:
        ...
        logger.info("📊 Final Merge Statistics:")
        logger.info(f"   ⏱️  Total Runtime: {runtime:.1f}s")
        logger.info(
            f"   📊 Total Input: {self.stats.total_left_results + self.stats.total_right_results} results"
        )
