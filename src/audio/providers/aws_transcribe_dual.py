...
class AWSTranscribeDualProvider(TranscriptionProvider):
    ...
    def __init__(
        ...
        logger.info(
            f"🏗️ AWSTranscribeDualProvider initialized: region={region}, language={language_code}"
        )
    ...
    async def start_stream(self, audio_config: AudioConfig) -> None:
        ...
        """
        try:
            logger.info("🚀 Dual AWS: Starting dual transcription streams")
            logger.info(
                f"🚀 Dual AWS: Audio config - {audio_config.channels} channels, {audio_config.sample_rate}Hz"
            )
    ...
    async def send_audio(self, audio_chunk: bytes) -> None:
        ...
        try:
            ...
            if not split_result.split_successful:
                logger.error(
                    f"❌ Dual AWS: Channel splitting failed: {split_result.error_message}"
                )
            ...
            if self.total_chunks_processed <= 10:
                ...
                logger.info(
                    f"   🎚️  Left: {split_result.left_metrics.activity_level} - {len(split_result.left_channel)} bytes"
                )
                logger.info(
                    f"   🎚️  Right: {split_result.right_metrics.activity_level} - {len(split_result.right_channel)} bytes"
                )
                ...
                if (
                    ...
                    logger.warning(
                        "⚠️ Left channel (Speaker A) silent - only processing right channel"
                    )
                elif split_result.right_metrics.is_silent:
                    logger.warning(
                        "⚠️ Right channel (Speaker B) silent - only processing left channel"
                    )
    ...
    async def _send_to_channel(
        ...
        try:
            ...
            if channel.error_count <= 5:  # Avoid log spam
                logger.error(
                    f"❌ {channel_name} channel: Send error #{channel.error_count}: {e}"
                )
    ...
    async def _handle_channel_failure(
        ...
        logger.warning(
            f"⚠️ {channel_name} channel: Failed after {failed_channel.error_count} errors"
        )
        ...
        if not self.fallback_mode:
            if left_active and not right_active:
                logger.warning(
                    "🔄 Dual AWS: Activating fallback mode - using only left channel (Speaker A)"
                )
                ...
                logger.warning(
                    "🔄 Dual AWS: Activating fallback mode - using only right channel (Speaker B)"
                )
                self.fallback_mode = True
                self.fallback_channel = "right"
                self.stats["fallback_activations"] += 1
                ...
                logger.error(
                    "❌ Dual AWS: Both channels failed - transcription unavailable"
                )
                self.is_active = False
    ...
    async def get_transcription(self) -> AsyncGenerator[TranscriptionResult, None]:
        ...
        if collection_tasks:
            logger.info(
                f"🔊 Dual AWS: Starting result collection from {len(collection_tasks)} channels"
            )
    ...
    async def _collect_channel_results(
        ...
        try:
            logger.info(
                f"🔊 {channel_name.title()} channel: Starting result collection for {speaker_label}"
            )
            ...
            async for result in provider.get_transcription():
                ...
                if self.result_queue:
                    ...
                    logger.debug(
                        f"📝 {speaker_label}: '{result.text}' (confidence: {result.confidence:.2f})"
                    )

        except asyncio.CancelledError:
            logger.info(
                f"🛑 {channel_name.title()} channel: Result collection cancelled"
            )
        except Exception as e:
            logger.error(
                f"❌ {channel_name.title()} channel: Result collection error: {e}"
            )
    ...
    async def _check_channel_health(
        ...
        if channel.last_result_time > 0 and time_since_last_result > 60.0:  # 60 seconds
            logger.warning(
                f"⚠️ {channel_name} channel: No results for {time_since_last_result:.0f}s"
            )
    ...
    def _log_channel_statistics(self) -> None:
        ...
        logger.info(
            f"   🎚️  Left Channel: {self.left_channel.results_received} results, "
            f"{self.left_channel.bytes_sent:,} bytes sent, {self.left_channel.error_count} errors"
        )
        logger.info(
            f"   🎚️  Right Channel: {self.right_channel.results_received} results, "
            f"{self.right_channel.bytes_sent:,} bytes sent, {self.right_channel.error_count} errors"
        )
        logger.info(
            f"   📈 Total Results: {self.stats['merged_results']} merged from {self.total_chunks_processed} chunks"
        )
        logger.info(
            f"   🔄 Fallback Mode: {self.fallback_mode} ({'active on ' + self.fallback_channel if self.fallback_mode else 'disabled'})"
        )

        # Channel splitter statistics
        splitter_stats = self.channel_splitter.get_statistics()
        logger.info(
            f"   🔀 Channel Splitter: {splitter_stats['left_silence_rate']:.1f}% left silence, "
    ...
    def _log_final_statistics(self) -> None:
        ...
        logger.info("📊 Final Dual AWS Statistics:")
        logger.info(f"   ⏱️  Total Runtime: {runtime:.1f}s")
        logger.info(f"   📦 Audio Chunks: {self.total_chunks_processed}")
        logger.info(
            f"   📝 Results: Left={self.stats['left_results']}, Right={self.stats['right_results']}, Merged={self.stats['merged_results']}"
        )
        logger.info(
            f"   ❌ Errors: Left={self.stats['left_errors']}, Right={self.stats['right_errors']}, Split={self.stats['split_errors']}"
        )
