...
class AzureSpeechProvider(TranscriptionProvider):
    ...
    def __init__(
        ...
        logger.info(f"🔵 AzureSpeechProvider initialized: region={region}, language={language_code}, diarization={enable_speaker_diarization}")
    ...
    async def start_stream(self, audio_config: AudioConfig) -> None:
        ...
        try:
            logger.info(f"🚀 Starting Azure Speech stream (language: {self.language_code}, sample_rate: {audio_config.sample_rate})")
            ...
            if self.enable_speaker_diarization:
                ...
                logger.info(f"🎙️ Azure speaker diarization enabled with max {self.max_speakers} speakers")
    ...
    def _setup_event_handlers(self):
            ...
            try:
                if evt.result.reason == speechsdk.ResultReason.RecognizingSpeech:
                    ...
                    if text.strip():
                        ...
                        logger.debug(f"🔵 Azure partial result: '{text}' (speaker: {speaker_id})")
            ...
            try:
                if evt.result.reason == speechsdk.ResultReason.RecognizedSpeech:
                    ...
                    if text.strip():
                        ...
                        # Put result in queue for async consumption
                        asyncio.create_task(self._queue_result(transcription_result))
                        logger.info(f"💬 Azure final result: '{text}' (speaker: {speaker_id}, confidence: {confidence:.2f})")
        ...
        def canceled_handler(evt):
            ...
            logger.warning(f"🚫 Azure Speech recognition canceled: {evt.result.cancellation_details}")
            self._is_connected = False
    ...
    async def send_audio(self, audio_chunk: bytes) -> None:
        ...
        try:
            ...
            logger.debug(f"📡 Sent audio chunk to Azure Speech: {len(audio_chunk)} bytes")
