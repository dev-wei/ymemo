...
class AWSTranscribeHandler(TranscriptResultStreamHandler):
    ...
    async def handle_transcript_event(self, transcript_event: TranscriptEvent):
        ...
        logger.debug(f"📥 AWS Handler: Received transcript event from AWS Transcribe")
        
        # Enhanced event analysis for debugging
        self._analyze_transcript_event(transcript_event)
        
        results = transcript_event.transcript.results
        logger.debug(f"📋 AWS Handler: Processing {len(results)} results from transcript event")
        ...
        for result in results:
            ...
            if hasattr(result, 'channel_labels') and result.channel_labels:
                ...
                for channel in result.channel_labels.channels:
                    if hasattr(channel, 'channel_label'):
                        ...
                        logger.info(f"🎙️ AWS Handler: Channel {channel.channel_label} → {speaker_id}")
                        break
            ...
            # Method 2: Fallback to channel_id (alternative AWS approach)
                ...
                logger.info(f"🎙️ AWS Handler: Channel ID {result.channel_id} → {speaker_id}")
            ...
            # Method 3: Fallback to item-level speaker labels (speaker diarization)
                ...
                for item in alternative.items:
                    if hasattr(item, 'speaker') and item.speaker:
                        ...
                        logger.debug(f"🎙️ AWS Handler: Item speaker {item.speaker} → {speaker_id}")
                        break
            ...
            logger.info(f"💬 AWS Handler: '{text}' (partial: {is_partial}, confidence: {confidence:.2f}, speaker: {speaker_id})")
    ...
    def _analyze_transcript_event(self, transcript_event: TranscriptEvent):
        ...
        try:
            ...
            if should_log_analysis or result_count > 0:
                ...
                logger.info(f"   📅 Time since last result: {current_time - self._last_result_time:.1f}s")
                logger.info(f"   📅 Time since non-empty result: {current_time - self._last_non_empty_result_time:.1f}s")
                logger.info(f"   📋 Event structure: transcript={has_transcript}, results={has_results}")
                ...
                if has_transcript:
                    ...
                    if has_results and result_count > 0:
                        ...
                        logger.info(f"   📋 Results array exists but is empty")
        ...
        try:
            for i, result in enumerate(results[:3]):  # Log first 3 results max
                ...
                if hasattr(result, 'alternatives') and result.alternatives:
                    ...
                    logger.info(f"     💬 Text: '{transcript_text}' (conf: {confidence:.3f})")
    ...
    def _log_channel_identification_details(self, result, result_index: int):
        ...
        try:
            ...
            if hasattr(result, 'channel_labels') and result.channel_labels:
                logger.info(f"     🎙️ Channel Labels Found!")
            ...
            # Method 3: Check for other channel-related attributes
                ...
                if channel_attrs:
                    ...
                    logger.info(f"     ⚠️ No channel identification found in result #{result_index}")
    ...
    def _log_empty_result_analysis(self, transcript_event: TranscriptEvent):
        ...
        try:
            ...
            if should_log:
                logger.info(f"🔍 AWS Empty Result Analysis (#{self._empty_result_count}):")
                logger.info(f"   ⏱️  Duration: {time_since_first_empty:.1f}s of empty results")
                logger.info(f"   📊 Rate: {self._empty_result_count / time_since_first_empty:.1f} empty results/second")
                
                # Check transcript event structure when empty
                if hasattr(transcript_event, 'transcript'):
                    transcript = transcript_event.transcript
                    logger.info(f"   📋 Transcript exists but results empty")
                    ...
                    if transcript_type_attrs:
                        logger.info(f"   📝 Available attributes: {transcript_type_attrs}")
                    ...
                    logger.warning(f"   ❌ Transcript event has no transcript attribute!")
                
                # Critical warning if too many consecutive empty results
                if self._empty_result_count > 200:
                    logger.warning(f"⚠️ AWS Handler: {self._empty_result_count} consecutive empty results! "
...
class AWSTranscribeProvider(TranscriptionProvider):
    ...
    def __init__(
        ...
        logger.info(f"🔧 AWS Transcribe: Connection strategy={self.connection_strategy}, dual_fallback={self.dual_fallback_enabled}, balance_threshold={self.channel_balance_threshold}")
        logger.info(f"🧪 AWS Transcribe: Dual connection test mode={self.dual_connection_test_mode}")
        if self.dual_save_split_audio or self.dual_save_raw_audio:
            logger.info(f"🎵 AWS Transcribe: Audio saving ENABLED (path: {self.dual_audio_save_path}, duration: {self.dual_audio_save_duration}s)")
            if self.dual_save_split_audio:
                logger.info(f"🎵 AWS Transcribe: Split audio saving enabled")
    ...
    def _determine_connection_strategy(self, audio_config) -> str:
        ...
        """
        logger.info(f"🔍 AWS Connection Strategy: Analyzing audio config - channels={audio_config.channels}, strategy={self.connection_strategy}")
        ...
        if self.connection_strategy == 'single':
            ...
            if audio_config.channels < 2:
                logger.warning("⚠️ AWS Connection Strategy: Dual mode requested but audio is mono - falling back to single")
                return 'single_connection'
            logger.info("🔧 AWS Connection Strategy: Forced dual connection mode")
            return 'dual_connection'
        
        # Auto mode - intelligent detection
        logger.info("🤖 AWS Connection Strategy: Auto mode - analyzing audio characteristics")
        
        # Single channel always uses single connection
        if audio_config.channels == 1:
            logger.info("🔧 AWS Connection Strategy: Single channel detected → single connection")
            return 'single_connection'
        ...
        if audio_config.channels == 2:
            logger.info("🔧 AWS Connection Strategy: Stereo detected → single connection with AWS dual-channel support")
            logger.info("🔧 AWS Connection Strategy: Dual connection available as fallback if channel imbalance detected")
            return 'single_connection'
        
        # More than 2 channels - not currently supported
        logger.error(f"❌ AWS Connection Strategy: Unsupported channel count: {audio_config.channels}")
    ...
    def _initialize_dual_connection_components(self, audio_config) -> None:
        ...
        logger.info(f"🔧 AWS Dual Connection: Channel splitter config - enable_saving={enable_saving}, path={save_path}, duration={save_duration}")
    ...
    def _validate_and_align_chunk(self, audio_chunk: bytes) -> tuple[bytes, dict]:
        ...
        if chunk_size % 4 != 0:
            ...
            if self._audio_chunk_count <= 20:
                logger.warning(f"⚠️ AWS Chunk Alignment: Original {chunk_size} bytes → "
    ...
    def _add_dual_channel_optimizations(self, stream_params: dict) -> None:
        ...
        if optimizations_added:
            logger.info(f"🚀 AWS Dual-Channel Optimizations: {', '.join(optimizations_added)}")
        else:
            logger.info(f"🚀 AWS Dual-Channel: Using standard channel identification parameters")
    ...
    def _validate_dual_channel_config(self, stream_params: dict) -> None:
        ...
        if recommendations:
            logger.info(f"💡 AWS Dual-Channel Recommendations: {'; '.join(recommendations)}")
        
        if not issues:
            logger.info(f"✅ AWS Dual-Channel configuration validation passed")
    ...
    def _log_channel_quality_summary(self, dual_channel_analysis: dict) -> None:
        ...
        try:
            ...
            logger.info(f"   🎚️  Channel Activity: Source A = {source_a_activity}, Source B = {source_b_activity}")
            ...
            if balance_status == 'well_balanced':
                logger.info(f"   ⚖️  Channel Balance: ✅ {balance_status} ({balance_ratio:.3f})")
            else:
                logger.info(f"   ⚖️  Channel Balance: ⚠️  {balance_status} ({balance_ratio:.3f})")
            
            # Critical issues that could explain incomplete transcription
            issues = balance.get('issues', [])
            if issues:
                logger.warning(f"   🚨 Channel Issues Affecting AWS Transcription:")
                for issue in issues:
            ...
            if source_a_avg > 0 and source_b_avg > 0:
                ...
                if amp_ratio > 5.0:  # One channel 5x louder than other
                    logger.warning(f"   📊 Amplitude Imbalance: {amp_ratio:.1f}x difference may cause AWS to ignore quieter channel")
                else:
                    logger.info(f"   📊 Amplitude Balance: {amp_ratio:.1f}x difference (acceptable)")
            
            # Channel silence warnings
            if source_a.get('is_silent', False):
                logger.warning(f"   🔇 Source A (Left) Silent: AWS will only process Source B (Right) channel")
            elif source_b.get('is_silent', False):
                logger.warning(f"   🔇 Source B (Right) Silent: AWS will only process Source A (Left) channel")
    ...
    def _log_aws_stream_configuration(self, stream_params: dict, audio_config: AudioConfig) -> None:
        ...
        logger.info(f"   🏷️  Provider: AWS Transcribe Streaming")
        logger.info(f"   🌍 Region: {self.region}")
        logger.info(f"   🗣️  Language: {stream_params.get('language_code', 'N/A')}")
        logger.info(f"   📡 Sample Rate: {stream_params.get('media_sample_rate_hz', 'N/A')} Hz")
        logger.info(f"   🎵 Media Encoding: {stream_params.get('media_encoding', 'N/A')}")
        logger.info(f"   🎚️  Channels: {stream_params.get('number_of_channels', 1)}")
        logger.info(f"   🔍 Channel ID Enabled: {stream_params.get('enable_channel_identification', False)}")
        ...
        if aws_features:
            ...
            logger.info(f"   ✨ Features: Basic transcription only")
        
        # Log original audio input configuration
        logger.info(f"   📦 Original Audio Config:")
        ...
        logger.info(f"   📈 Expected Data Rates:")
        ...
        logger.info(f"   🔗 AWS Service: transcribestreaming.{self.region}.amazonaws.com")
    ...
    def _validate_aws_stream_params(self, stream_params: dict) -> dict:
        ...
        if (stream_params.get('enable_channel_identification') and 
            ...
            logger.info("✅ Dual-channel PCM configuration detected - optimal for speaker separation")
    ...
    async def start_stream(self, audio_config: AudioConfig) -> None:
        ...
        try:
            ...
            logger.info(f"🎯 AWS Transcribe: Selected connection mode: {connection_mode}")
    ...
    async def _start_single_connection_stream(self, audio_config: AudioConfig) -> None:
        ...
        try:
            ...
            logger.info(f"🎯 Starting stream transcription (language: {self.language_code}, sample_rate: {audio_config.sample_rate}, channels: {audio_config.channels})")
            ...
            if audio_config.channels == 1:
                ...
                logger.info("🎯 AWS Transcribe: Mono input - standard transcription mode")
                ...
                logger.info("🎯 AWS Transcribe: Dual-channel input - enabled channel identification for speaker separation")
            elif audio_config.channels > 2:
                # This shouldn't happen with device filtering to 1-2 channels only
                logger.warning(f"⚠️ AWS Transcribe: Received {audio_config.channels} channels. Only 1-2 channels supported.")
            ...
            logger.info("🔄 AWS Transcribe: Handler and health monitor started using AWS pattern")
        ...
        if self.stream and self.stream.input_stream:
            try:
                ...
                if (self.dual_fallback_enabled and 
                    ...
                    logger.warning("🔄 AWS Fallback: Fallback conditions detected, will attempt to switch to dual connection")
                    ...
                    if not hasattr(self, '_stored_audio_config'):
                        ...
                        logger.warning("⚠️ AWS Fallback: Fallback needed but cannot switch during send_audio - consider switching at stream level")
                ...
                if self._audio_chunk_count <= 100 and self._audio_chunk_count % 10 == 0:
                    if "error" in audio_analysis:
                        logger.warning(f"⚠️ AWS Audio Analysis Error: {audio_analysis['error']}")
                    else:
                        logger.info(f"🎵 AWS Audio Analysis (chunk #{self._audio_chunk_count}):")
                        logger.info(f"   📊 Overall - Max: {audio_analysis.get('max_amplitude', 'N/A')}, "
                                   f"Avg: {audio_analysis.get('avg_amplitude', 'N/A'):.1f}")
                        logger.info(f"   🔇 Silent: {audio_analysis.get('is_silent', 'N/A')} "
                                   f"(threshold: {audio_analysis.get('silence_threshold', 'N/A')})")
                        logger.info(f"   📦 Samples: {audio_analysis.get('sample_count', 'N/A')}, "
                        ...
                        if alignment:
                            if not alignment.get('is_aligned', True):
                                logger.warning(f"   ⚠️ Alignment: {alignment['original_size']} → "
                                             f"{alignment['aligned_size']} bytes (+{alignment['padding_added']} padding)")
                            elif alignment.get('chunk_valid_for_aws'):
                                logger.debug(f"   ✅ Chunk aligned: {alignment['aligned_size']} bytes "
                        ...
                        if dual_channel.get('is_dual_channel'):
                            ...
                            logger.info(f"   🎚️  Source A (Left): {source_a.get('activity_level', 'N/A')} - "
                                       f"Max: {source_a.get('max_amplitude', 'N/A')}, "
                                       f"Avg: {source_a.get('avg_amplitude', 'N/A'):.1f}")
                            logger.info(f"   🎚️  Source B (Right): {source_b.get('activity_level', 'N/A')} - "
                                       f"Max: {source_b.get('max_amplitude', 'N/A')}, "
                                       f"Avg: {source_b.get('avg_amplitude', 'N/A'):.1f}")
                            logger.info(f"   ⚖️  Balance: {balance.get('balance_status', 'N/A')} "
                                       f"(ratio: {balance.get('balance_ratio', 0):.3f})")
                            
                            # Log critical issues
                            issues = balance.get('issues', [])
                            if issues:
                                logger.warning(f"   ⚠️ Channel Issues: {', '.join(issues)}")
                            
                            recommendation = balance.get('recommendation', '')
                            if recommendation and 'optimal' not in recommendation.lower():
                                logger.info(f"   💡 Recommendation: {recommendation}")
                        elif "error" in dual_channel:
                            logger.warning(f"   ⚠️ Dual-channel analysis error: {dual_channel['error']}")
                        else:
                            logger.info(f"   🔇 Single-channel audio detected")
                ...
                if self._audio_chunk_count % 100 == 0:
                    ...
                    logger.info(f"📊 AWS Audio Summary (chunk #{self._audio_chunk_count}):")
                    logger.info(f"   🔇 Overall silence rate: {silence_rate:.1f}% ({self._silence_chunks}/{self._audio_chunk_count})")
                    logger.info(f"   📈 Average audio level: {avg_level:.1f}")
                    logger.info(f"   🎵 Total samples analyzed: {self._total_audio_samples_analyzed:,}")
                    ...
                    if silence_rate > 80:
                        logger.warning(f"⚠️ AWS Transcribe: High silence rate ({silence_rate:.1f}%) - "
                    ...
                    if alignment and not alignment.get('is_aligned', True):
                        ...
                        logger.info(f"   ⚠️ Alignment: ~{total_padding:.0f} padding bytes added in last 100 chunks")
                    
                    logger.info(f"📡 AWS Transcribe: Audio chunk #{self._audio_chunk_count} - {chunk_size} bytes sent directly to AWS")
    ...
    async def _start_dual_connection_stream(self, audio_config: AudioConfig) -> None:
        ...
        logger.info(f"🚀 AWS Dual Connection: Starting dual connection stream in test mode: {test_mode}")
        ...
        try:
            ...
            if test_mode in ['left_only', 'full']:
                ...
                logger.info("🧪 AWS Dual Connection: Left channel provider DISABLED in test mode")
            ...
            if test_mode in ['right_only', 'full']:
                ...
                logger.info("🧪 AWS Dual Connection: Right channel provider DISABLED in test mode")
            ...
            if components['left_provider']:
                ...
                logger.info("🧪 AWS Dual Connection: Left channel stream SKIPPED in test mode")
            ...
            if components['right_provider']:
                ...
                logger.info("🧪 AWS Dual Connection: Right channel stream SKIPPED in test mode")
            ...
            if test_mode == 'left_only':
                logger.info("🧪 AWS Dual Connection: TEST MODE - Only left channel (Source A) is active")
            elif test_mode == 'right_only':
                logger.info("🧪 AWS Dual Connection: TEST MODE - Only right channel (Source B) is active")
            else:
            ...
            if self.dual_save_split_audio or self.dual_save_raw_audio:
                logger.info(f"🎵 AWS Dual Connection: Audio saving is ENABLED")
                ...
                logger.info(f"🎵 AWS Dual Connection: Audio saving is DISABLED")
            
            logger.info("✅ AWS Dual Connection: Dual connection stream started successfully")
            
        except Exception as e:
            logger.error(f"❌ AWS Dual Connection: Failed to start dual connection stream: {e}")
    ...
    async def _send_audio_dual_connection(self, audio_chunk: bytes) -> None:
        ...
        try:
            ...
            if not split_result.split_successful:
                logger.error(f"❌ AWS Dual Connection: Channel splitting failed: {split_result.error_message}")
                return
            ...
            if test_mode in ['left_only', 'full'] and components['left_provider']:
                try:
                    ...
                    logger.error(f"❌ AWS Dual Connection: Left channel send failed: {e}")
                    components['error_handler'].record_connection_failure("left", e)
            elif test_mode in ['left_only', 'full']:
                logger.debug(f"🧪 AWS Dual Connection: Left channel audio dropped (provider not active)")
            ...
            if test_mode in ['right_only', 'full'] and components['right_provider']:
                try:
                    ...
                    logger.error(f"❌ AWS Dual Connection: Right channel send failed: {e}")
                    components['error_handler'].record_connection_failure("right", e)
            elif test_mode in ['right_only', 'full']:
                logger.debug(f"🧪 AWS Dual Connection: Right channel audio dropped (provider not active)")
            ...
            if self._dual_audio_chunk_count % 100 == 0:  # Every 100 chunks
                ...
                logger.info(f"🧪 AWS Dual Connection (#{self._dual_audio_chunk_count}): Test mode={test_mode}, Active channels: {', '.join(active_channels)}")
                ...
                if self.dual_save_split_audio and components['channel_splitter'].audio_saver:
                    if components['channel_splitter'].audio_saver.is_active:
                        logger.info(f"🎵 AWS Dual Connection: Audio saving in progress...")
                    else:
                        logger.info(f"🎵 AWS Dual Connection: Audio saving completed or not started")
    ...
    async def _get_transcription_dual_connection(self) -> AsyncGenerator[TranscriptionResult, None]:
        ...
        logger.info(f"🔊 AWS Dual Connection: Starting dual transcription generator in test mode: {test_mode}")
        ...
        async def collect_left_results():
            ...
            if not components['left_provider']:
                logger.info("🧪 AWS Dual Connection: Left channel collection SKIPPED (provider not active)")
                return
                
            try:
                logger.info("🔊 AWS Dual Connection: Starting left channel result collection...")
                async for result in components['left_provider'].get_transcription():
                    ...
                    logger.debug(f"🧪 AWS Dual Connection: Left result: '{result.text}' (confidence: {result.confidence:.2f})")
            except Exception as e:
                logger.error(f"❌ AWS Dual Connection: Left channel collection failed: {e}")
                components['error_handler'].record_connection_failure("left", e)
        
        async def collect_right_results():
            """Collect results from right channel provider."""
            if not components['right_provider']:
                logger.info("🧪 AWS Dual Connection: Right channel collection SKIPPED (provider not active)")
                return
                
            try:
                logger.info("🔊 AWS Dual Connection: Starting right channel result collection...")
                async for result in components['right_provider'].get_transcription():
                    ...
                    logger.debug(f"🧪 AWS Dual Connection: Right result: '{result.text}' (confidence: {result.confidence:.2f})")
            except Exception as e:
                logger.error(f"❌ AWS Dual Connection: Right channel collection failed: {e}")
                components['error_handler'].record_connection_failure("right", e)
        ...
        if not active_tasks:
            logger.error("❌ AWS Dual Connection: No active channels configured for result collection")
            return
        
        logger.info(f"🧪 AWS Dual Connection: Started {len(active_tasks)} result collection tasks")
        
        try:
            # Yield merged results as they become available
            async for merged_result in result_merger.get_merged_results():
                logger.info(f"🧪 AWS Dual Connection: Test mode result: {merged_result.speaker_id}: '{merged_result.text}' (confidence: {merged_result.confidence:.2f})")
                yield merged_result
    ...
    async def _stop_dual_connection_stream(self) -> None:
        ...
        """
        test_mode = self.dual_connection_test_mode
        logger.info(f"🛑 AWS Dual Connection: Stopping dual connection stream (test mode: {test_mode})...")
        ...
        try:
            ...
            if components['result_merger']:
                try:
                    ...
                    logger.warning(f"⚠️ AWS Dual Connection: Error stopping result merger: {e}")
            
            # Stop active channel providers based on test mode
            if components['left_provider'] and test_mode in ['left_only', 'full']:
                try:
                    logger.info("🛑 AWS Dual Connection: Stopping left channel provider...")
                    await components['left_provider'].stop_stream()
                except Exception as e:
                    logger.warning(f"⚠️ AWS Dual Connection: Error stopping left provider: {e}")
            elif components['left_provider']:
                logger.info("🧪 AWS Dual Connection: Left provider was inactive (test mode)")
            
            if components['right_provider'] and test_mode in ['right_only', 'full']:
                try:
                    logger.info("🛑 AWS Dual Connection: Stopping right channel provider...")
                    await components['right_provider'].stop_stream()
                except Exception as e:
                    logger.warning(f"⚠️ AWS Dual Connection: Error stopping right provider: {e}")
            elif components['right_provider']:
                logger.info("🧪 AWS Dual Connection: Right provider was inactive (test mode)")
            ...
            if components['error_handler']:
                try:
                    ...
                    logger.warning(f"⚠️ AWS Dual Connection: Error stopping error handler: {e}")
            
            logger.info("✅ AWS Dual Connection: Dual connection stream stopped successfully")
            
        except Exception as e:
            logger.error(f"❌ AWS Dual Connection: Error during dual connection stop: {e}")
            ...
            if (self._dual_connection_components and 
                ...
                try:
                    ...
                    if save_stats:
                        logger.info(f"🎵 AWS Dual Connection: Split audio saving stopped during cleanup")
                except Exception as e:
                    logger.error(f"❌ AWS Dual Connection: Error stopping split audio saving: {e}")
            ...
            if self._raw_audio_saver and self._raw_audio_saver.is_active():
                try:
                    ...
                    logger.info(f"🎵 AWS Dual Connection: Raw audio saving stopped during cleanup")
                    logger.info(f"   📁 Raw audio file: {raw_stats.get('file_path', 'N/A')}")
                except Exception as e:
                    logger.error(f"❌ AWS Dual Connection: Error stopping raw audio saving: {e}")
    ...
    def _log_raw_audio_input(self, audio_chunk: bytes) -> None:
        ...
        if self._raw_audio_chunk_count % log_interval == 0:
            ...
            logger.info(f"   📊 Data rate: {actual_bytes_per_second:,.0f} bytes/sec (expected: {expected_bytes_per_second:,})")
            ...
            if audio_analysis:
                ...
                if dual_analysis.get('is_dual_channel'):
                    ...
                    logger.warning(f"      - ⚠️  Audio appears to be MONO, not stereo!")
    ...
    def _initialize_raw_audio_saving(self, audio_config) -> None:
        ...
        try:
            ...
            if self._raw_audio_saver.start_recording():
                logger.info(f"🎵 AWS Dual Connection: Raw audio saving started")
                logger.info(f"   📁 File: {raw_audio_path}")
                logger.info(f"   📋 Format: {audio_config.sample_rate}Hz, {audio_config.channels}ch, 16-bit")
            else:
                logger.error(f"❌ AWS Dual Connection: Failed to start raw audio recording")
                self._raw_audio_saver = None
                
        except Exception as e:
            logger.error(f"❌ AWS Dual Connection: Failed to initialize raw audio saving: {e}")
            self._raw_audio_saver = None
    ...
    def _should_fallback_to_dual_connection(self, audio_analysis: Dict[str, Any]) -> bool:
        ...
        if dual_channel.get('is_dual_channel'):
            ...
            if imbalance_ratio > self.channel_balance_threshold:
                logger.warning(f"⚠️ AWS Fallback: Severe channel imbalance detected: {balance_ratio:.3f} (threshold: {self.channel_balance_threshold})")
                logger.warning(f"⚠️ AWS Fallback: Will attempt fallback to dual connection after this send_audio cycle")
                return True
            ...
            if source_a_silent and not source_b_silent:
                logger.warning("⚠️ AWS Fallback: Source A (left) is silent while Source B (right) has audio")
                logger.warning("⚠️ AWS Fallback: Will attempt fallback to dual connection after this send_audio cycle")
                return True
            elif source_b_silent and not source_a_silent:
                logger.warning("⚠️ AWS Fallback: Source B (right) is silent while Source A (left) has audio")
                logger.warning("⚠️ AWS Fallback: Will attempt fallback to dual connection after this send_audio cycle")
                return True
    ...
    async def _attempt_fallback_to_dual_connection(self, audio_config: AudioConfig) -> bool:
        ...
        if audio_config.channels < 2:
            logger.warning("⚠️ AWS Fallback: Cannot fallback to dual connection with mono audio")
            return False
        
        logger.warning("🔄 AWS Fallback: Attempting fallback from single to dual connection...")
        ...
        try:
            ...
            try:
                logger.info("🔄 AWS Fallback: Attempting to restore single connection...")
                self._connection_mode = 'single_connection'
                await self._start_single_connection_stream(audio_config)
                logger.info("✅ AWS Fallback: Restored single connection after failed fallback")
            except Exception as restore_error:
                logger.error(f"❌ AWS Fallback: Failed to restore single connection: {restore_error}")
    ...
    def _track_transcription_quality(self, result: TranscriptionResult) -> None:
        ...
        if result.confidence is not None and result.confidence > 0.0:
            logger.debug(f"📊 AWS Quality: Got result with confidence {result.confidence:.3f}: '{result.text}'")
        elif result.text.strip():
            logger.debug(f"📊 AWS Quality: Got result with 0.000 confidence (normal for AWS): '{result.text}'")
