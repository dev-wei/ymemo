...
class AudioChannelProcessor:
    ...
    def __init__(self, mixing_strategy: MixingStrategy = MixingStrategy.AVERAGE):
        ...
        logger.info(
            f"🔄 AudioChannelProcessor: Initialized with {mixing_strategy.value} mixing strategy"
        )
    ...
    def convert_channels(
        ...
        raise NotImplementedError(
            f"Channel conversion {source_channels}→{target_channels} not yet implemented"
        )
    ...
    def convert_to_optimal_channels(
        ...
        if source_channels <= 2:
            ...
            if source_channels == 1:
                ...
                logger.debug(
                    f"🔄 ChannelProcessor #{self._conversion_count}: No conversion needed (already mono)"
                )
                return audio_data, 1
            # 2 channels → mix down to mono
            logger.debug(
                f"🔄 ChannelProcessor #{self._conversion_count}: Converting 2ch → 1ch (mono)"
            )
            ...
            if self._debug_sample_logging and self._conversion_count <= 10:
                ...
                if "error" not in analysis:
                    logger.info(
                        f"🔍 ChannelProcessor Analysis #{self._conversion_count} (2ch→1ch):"
                    )
                    logger.info(
                        f"   📊 Amplitude: {analysis['input_avg_amplitude']:.1f} → "
                        f"{analysis['output_avg_amplitude']:.1f} (ratio: {analysis['amplitude_ratio']:.3f})"
                    )
                    logger.info(
                        f"   📦 Samples: {analysis['input_sample_count']} → {analysis['output_sample_count']}"
                    )
                    logger.info(
                        f"   📏 Size: {len(audio_data)} → {len(converted_data)} bytes ({analysis['size_reduction']:.3f})"
                    )
                else:
                    logger.warning(
                        f"⚠️ ChannelProcessor Analysis #{self._conversion_count}: {analysis['error']}"
                    )
        ...
        if source_channels <= 4:
            ...
            logger.debug(
                f"🔄 ChannelProcessor #{self._conversion_count}: Converting {source_channels}ch → 2ch (dual-channel)"
            )
            ...
            if should_analyze:
                ...
                if "error" not in analysis:
                    logger.info(
                        f"🔍 ChannelProcessor Analysis #{self._conversion_count} ({source_channels}ch→2ch):"
                    )
                    logger.info(
                        f"   📊 Input amplitude: Max={analysis['input_max_amplitude']}, "
                        f"Avg={analysis['input_avg_amplitude']:.1f}"
                    )
                    logger.info(
                        f"   📊 Output amplitude: Max={analysis['output_max_amplitude']}, "
                        f"Avg={analysis['output_avg_amplitude']:.1f}"
                    )
                    logger.info(
                        f"   📊 Amplitude ratio: {analysis['amplitude_ratio']:.3f}"
                    )
                    logger.info(
                        f"   📦 Samples: {analysis['input_sample_count']} → {analysis['output_sample_count']}"
                    )
                    ...
                    if analysis["output_max_amplitude"] < 10:
                        logger.warning(
                            f"⚠️ ChannelProcessor #{self._conversion_count}: "
    ...
    def _process_int16(
        ...
        raise NotImplementedError(
            f"int16 conversion {source_channels}→{target_channels} not implemented"
        )
    ...
    def _process_int32(
        ...
        raise NotImplementedError(
            f"int32 conversion {source_channels}→{target_channels} not implemented"
        )
    ...
    def _process_float32(
        ...
        raise NotImplementedError(
            f"float32 conversion {source_channels}→{target_channels} not implemented"
        )
