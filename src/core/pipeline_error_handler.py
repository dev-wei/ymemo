...
class PipelineErrorHandler:
    ...
    async def handle_pipeline_operation(
        ...
        logger.info(
            f"🔄 Pipeline: Starting operation '{operation_name}' (timeout: {operation_timeout}s)"
        )
        ...
        try:
            ...
            logger.info(
                f"✅ Pipeline: Operation '{operation_name}' completed successfully in {duration:.2f}s"
            )
            ...
            if cleanup_callback:
                try:
                    ...
                    logger.error(
                        f"❌ Pipeline: Cleanup failed for '{operation_name}': {cleanup_error}"
                    )
            ...
            logger.debug(
                f"❌ Pipeline: Full traceback for '{operation_name}':\n{traceback.format_exc()}"
            )
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if attempt == self.max_retries:
                    logger.error(
                        f"❌ Pipeline: Operation '{operation_name}' failed after {self.max_retries} attempts"
                    )
                    break
                ...
                logger.warning(
                    f"⚠️ Pipeline: Operation '{operation_name}' failed (attempt {attempt}/{self.max_retries}), "
    ...
    def _record_error(self, operation_name: str, severity: ErrorSeverity):
        ...
        logger.info(
            f"📊 Pipeline: Error recorded for '{operation_name}' "
    ...
    async def safe_cleanup(
        ...
        for operation_name, cleanup_func in cleanup_operations.items():
            try:
                ...
                logger.error(
                    f"⏱️ Pipeline: Cleanup timeout for '{operation_name}' after {timeout_per_operation}s"
                )

            except Exception as e:
                results[operation_name] = False
                logger.error(
                    f"❌ Pipeline: Cleanup failed for '{operation_name}': {str(e)}"
                )
                logger.debug(
                    f"❌ Pipeline: Cleanup traceback for '{operation_name}':\n{traceback.format_exc()}"
                )
        ...
        logger.info(
            f"🧹 Pipeline: Cleanup summary: {successful_cleanups}/{total_cleanups} operations successful"
        )
    ...
    def should_circuit_break(
        ...
        logger.warning(
            f"⚡ Pipeline: Circuit breaker triggered for '{operation_name}' "
