...
class AudioProcessorFactory:
    ...
    def create_transcription_provider(
        ...
        **config
        ...
        try:
            ...
            logger.error(f"❌ Factory: Failed to create transcription provider '{provider_name}': {e}")
    ...
    def create_audio_capture_provider(
        ...
        **config
        ...
        try:
            logger.info(f"🏭 Factory: Creating audio capture provider '{provider_name}' with config keys: {list(config.keys())}")
            ...
            if hasattr(provider_instance, '_instance_id'):
                ...
                logger.info(f"✅ Factory: Successfully created {provider_name} audio capture provider")
            ...
            logger.error(f"❌ Factory: Failed to create audio capture provider '{provider_name}': {e}")
    ...
    def register_transcription_provider(
        ...
        logger.info(f"✅ Factory: Registered transcription provider '{name}' -> {provider_class.__name__}")
    ...
    def register_audio_capture_provider(
        ...
        logger.info(f"✅ Factory: Registered audio capture provider '{name}' -> {provider_class.__name__}")
