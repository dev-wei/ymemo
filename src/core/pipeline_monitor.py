...
class PipelineMonitor:
    ...
    def record_stage_complete(
        ...
        logger.debug(
            f"📊 Stage completed: {stage.value} [{correlation_id}] - {duration_ms:.1f}ms ({'✅' if success else '❌'})"
        )
    ...
    def record_audio_chunk_processed(
        ...
        logger.debug(
            f"📊 Audio chunk: {chunk_size_bytes} bytes, {processing_time_ms:.1f}ms"
        )
    ...
    def _assess_pipeline_health(self) -> None:
        ...
        if len(self.health_history) > 1:
            ...
            if previous_health.overall_status != health.overall_status:
                logger.info(
                    f"🏥 Pipeline health changed: {previous_health.overall_status.value} → {health.overall_status.value}"
                )
    ...
    def _update_stage_metrics(
        ...
        logger.debug(
            f"📊 Stage: {stage.value} - {duration_ms:.1f}ms ({'✅' if success else '❌'})"
        )
    ...
    def _generate_session_monitoring_report(self) -> dict[str, Any]:
        ...
        logger.info(
            f"📊 Generated monitoring report for session {self.current_session_id}"
        )
        return report
