...
class ManagedResource:
    ...
    async def cleanup(self) -> bool:
        ...
        logger.info(
            f"🧹 Resource: Cleaning up '{self.resource_id}' (attempt {self.cleanup_attempts})"
        )
        ...
        try:
            ...
            logger.error(
                f"⏱️ Resource: Cleanup timeout for '{self.resource_id}' after {self.timeout}s"
            )
            return False
...
class ManagedTask:
    ...
    async def _execute_with_monitoring(self, coro):
        ...
        try:
            ...
            logger.info(
                f"✅ Task: '{self.task_id}' completed successfully in {duration:.2f}s"
            )
            ...
            if self.cleanup_on_cancel:
                try:
                    ...
                    logger.info(
                        f"🧹 Task: Cleanup completed for cancelled task '{self.task_id}'"
                    )
                except Exception as e:
                    logger.error(
                        f"❌ Task: Cleanup failed for cancelled task '{self.task_id}': {e}"
                    )
    ...
    async def cancel(self, timeout: float = 2.0) -> bool:
        ...
        try:
            ...
            logger.error(
                f"⏱️ Task: Cancellation timeout for '{self.task_id}' after {timeout}s"
            )
            return False
        except asyncio.CancelledError:
            logger.info(f"✅ Task: '{self.task_id}' cancelled successfully")
...
class ResourceManager:
    ...
    def register_resource(
        ...
        if resource_id in self.resources:
            logger.warning(
                f"⚠️ ResourceManager: Resource '{resource_id}' already registered, replacing"
            )
    ...
    def create_task(
        ...
        if task_id in self.tasks:
            logger.warning(
                f"⚠️ ResourceManager: Task '{task_id}' already exists, cancelling old task"
            )
            # Cancel old task
    ...
    async def cleanup_resource(self, resource_id: str) -> bool:
        ...
        if resource_id not in self.resources:
            logger.warning(
                f"⚠️ ResourceManager: Resource '{resource_id}' not found for cleanup"
            )
            return True
        ...
        if success:
            ...
            logger.info(
                f"🗑️ ResourceManager: Removed resource '{resource_id}' from registry"
            )
    ...
    async def cancel_task(self, task_id: str, timeout: float = 2.0) -> bool:
        ...
        if task_id not in self.tasks:
            logger.warning(
                f"⚠️ ResourceManager: Task '{task_id}' not found for cancellation"
            )
            return True
    ...
    async def cleanup_all(self, timeout_per_operation: float = 3.0) -> dict[str, bool]:
        ...
        if task_cancellations:
            ...
            for _i, (task_id, result) in enumerate(
                ...
                if isinstance(result, Exception):
                    ...
                    logger.error(
                        f"❌ ResourceManager: Task cancellation failed for '{task_id}': {result}"
                    )
                else:
        ...
        logger.info(
            f"🧹 ResourceManager: Cleaning up {len(self.resources)} resources..."
        )
        resource_cleanups = []
        ...
        if resource_cleanups:
            ...
            for _i, (resource_id, result) in enumerate(
                ...
                if isinstance(result, Exception):
                    ...
                    logger.error(
                        f"❌ ResourceManager: Resource cleanup failed for '{resource_id}': {result}"
                    )
                else:
        ...
        logger.info(
            f"🧹 ResourceManager: Cleanup completed - {successful}/{total} operations successful"
        )
    ...
    def cleanup_stale_resources(self, max_age_minutes: int = 30) -> int:
        ...
        if stale_resources:
            logger.info(
                f"🕰️ ResourceManager: Found {len(stale_resources)} stale resources"
            )
    ...
    async def managed_lifecycle(self):
        ...
        try:
            logger.info("🚀 ResourceManager: Starting managed lifecycle")
            yield self
        finally:
            logger.info("🛑 ResourceManager: Ending managed lifecycle, cleaning up...")
            await self.cleanup_all()
            logger.info("✅ ResourceManager: Managed lifecycle cleanup completed")
